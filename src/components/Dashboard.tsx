...
import { MonitoringStats, InterceptedAction } from "../types";
...
const Dashboard: React.FC<DashboardProps> = ({ stats, actions, onRefreshStats }) => {
  ...
  // Fake system load jitter
  React.useEffect(() => {
    const interval = setInterval(() => {
       setSimulatedLoad(Math.floor(Math.random() * 85) + 10);
    }, 1000);
    ...
    <div className="flex-1 flex w-full h-full gap-2">
      ...
      <div className="w-[320px] hud-panel flex flex-col shrink-0 gap-4 overflow-hidden">
         <div className="flex items-center justify-between border-b border-hud-cyan-border pb-2">
            ...
            <span className="hud-label text-[9px]">UPLINK: OK</span>
         </div>
         
         <div className="flex flex-col gap-2">
           <div className="flex justify-between items-end border-b border-hud-cyan-border/30 pb-1">
             <span className="hud-label text-hud-text-primary">CORE_TEMP</span>
             <span className="hud-data text-hud-amber">{32 + Math.floor(simulatedLoad / 5)}°C</span>
           </div>
           <div className="flex justify-between items-end border-b border-hud-cyan-border/30 pb-1">
             <span className="hud-label text-hud-text-primary">NET_PING</span>
             <span className="hud-data text-hud-cyan">{14 + Math.floor(Math.random() * 5)}ms</span>
           </div>
           <div className="flex justify-between items-end border-b border-hud-cyan-border/30 pb-1">
             ...
             <div className="w-24 h-2 bg-black/50 border border-hud-cyan-border relative">
                <div className="absolute top-0 left-0 bottom-0 bg-hud-cyan transition-all duration-300" style={{width: `${simulatedLoad}%`}} />
             </div>
           </div>
         </div>

         <div className="flex-1 overflow-y-auto mt-2 pointer-events-none">
            {/* Fake hex memory stream */}
            <div className="opacity-40 animate-pulse" style={{ animationDuration: '4s' }}>
              {Array.from({length: 15}).map((_, i) => (
                 <div key={i} className="hud-data text-[10px] text-hud-text-muted flex justify-between">
                   <span>0x{Math.floor(Math.random()*16777215).toString(16).padEnd(6, '0')}</span>
                   <span>{(Math.random() > 0.5) ? 'PASS' : 'CHK'}</span>
                   <span>{Math.floor(Math.random() * 900)}B</span>
                 </div>
              ))}
            </div>
         </div>
    ...
    return (
        <div className="absolute inset-0 flex items-center justify-center">
            ...
            <div className="relative w-80 h-80 rounded-full border border-hud-cyan/20 overflow-hidden bg-black/40">
                ...
                {/* Vertical Longitudes (Rotating Overlay) */}
                ...
                {/* Trajectory Arc (Mocked Exfiltration Path) */}
