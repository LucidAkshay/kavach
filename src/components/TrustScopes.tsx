...
const GhostModeBoard = () => {
    return (
        <div className="hud-card p-4 border border-hud-cyan-border flex flex-col gap-3 group relative overflow-hidden opacity-75">
            <div className="flex items-center justify-between z-10">
                <div className="flex items-center gap-2">
                    <span className="hud-data text-[12px] font-bold text-hud-text-primary">GHOST_MODE</span>
                    <span className="bg-hud-amber text-black hud-data text-[9px] px-1.5 py-0.5 font-bold tracking-widest">[v1.2 ROADMAP]</span>
                </div>
                <span className="hud-data text-[10px] font-bold text-hud-text-muted">
                    [ OFFLINE ]
                </span>
            </div>

            <span className="hud-label text-[9px] text-hud-text-muted max-w-[90%] z-10">
                [IN DEVELOPMENT] Will silently spoof blocked network requests for analysis without payload execution. Waiting on OS level network interception hooks.
            </span>

            <button
                disabled={true}
                className="mt-2 py-2 hud-data tracking-[0.2em] font-bold border transition-colors z-10 cursor-not-allowed border-hud-cyan-border/30 text-hud-cyan-border/50 bg-black"
            >
                &gt; FEATURE LOCKED &lt;
            </button>
        </div>
