...
const TerminalEntry = ({ action }: { action: InterceptedAction }) => {
  ...
  return (
    <div className={`hud-card p-2 flex flex-col gap-1 border-l-2 ${isHigh ? 'border-l-hud-crimson bg-hud-crimson-dim/30' : action.risk_level === 'Medium' ? 'border-l-hud-amber bg-hud-amber-dim/20' : 'border-l-hud-emerald'}`}>
      ...
      <div className="hud-data flex items-center gap-2">
        <span className="opacity-50 text-hud-cyan">{'>'}</span>
        <span className="hud-data">{intent}</span>
        {action.action_type.includes("CRITICAL_VELOCITY") && (
      ...
      <div className="flex justify-end mt-1 gap-2">
        ...
        <span className={`hud-label font-bold bg-black/50 px-2 py-0.5 border ${action.status === "approved" ? "text-hud-emerald border-hud-emerald" :
          ...
          {action.status === "pending" ? "AWAITING SIG..." : action.status.toUpperCase()}
        </span>
      </div>
    </div>
  );
